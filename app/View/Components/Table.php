...
class Table extends Component
{
    ...
    public function __construct(
        ...
        public mixed $prependActions = null,
                            ...
                            @foreach($rows as $k => $row)
                                ...
                                >
                                    ...
                                    @if($expandable)
                                        <td class="w-1 pe-0">
                                            @if(data_get($row, $expandableCondition))
                                                <x-mary-icon
                                                    ...
                                                    class="w-8 h-8 p-2 rounded-lg cursor-pointer bg-base-300"
                                     ...
                                     @if($prependActions)
                                        <td class="py-0 text-right">{{ $prependActions($row) }}</td>
                                    @endif
                                    ...
                                    @foreach($headers as $header)
                                        ...
                                        @if(isset(${"cell_".$temp_key}))
                                            <td @class([$cellClasses($row, $header), "p-0" => $hasLink($header)])>
                                                @if($hasLink($header))
                                                    <a href="{{ $redirectLink($row) }}" wire:navigate class="block px-4 py-3">
                                                @endif
                                    ...
                                    <!-- ACTIONS -->
                                    @if($actions)
                                        <td class="py-0 text-right">{{ $actions($row) }}</td>
                                    @endif
